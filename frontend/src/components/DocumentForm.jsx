...
const DocumentForm = forwardRef(({ templateFields, onChange }, ref) => {
    ...
    const renderInputField = (field, blockIndex = null) => {
         ...
         switch (field.fieldType) {
             ...
             case 'radio': return ( <div className={styles.radioGroup}> {field.options?.map((opt, idx) => { const val = typeof opt === 'string' ? opt : opt.value; const lbl = typeof opt === 'string' ? opt : opt.label; const id = `${inputName}-opt-${idx}`; return ( <label key={id} className={styles.radioContainer}> <input type="radio" id={id} name={inputName} value={val} checked={String(value) === String(val)} onChange={handleInputChange} className={styles.radioInput} /> <span className={styles.radioLabel}>{lbl}</span> </label> ); })} </div> );
                    ...
                    if (field.fieldType === 'repeatable') {
                        ...
                        return (
                            <div key={field.name} className={styles.formGroup}>
                                ...
                                {field.name === 'belge_email' && ( <small className={styles.emailInfoText}> Orijinal PDF belgenizin bir kopyası bu adrese gönderilecektir. </small> )}
                            </div>
                        );
                    }
                })}
